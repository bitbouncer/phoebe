...
main(int argc, char** argv)
{
    ...
    desc.add_options()
        ...
        ("delkeyfile", boost::program_options::value<std::string>(), "file delete keys from this file")
        ("noop", boost::program_options::value<std::string>(), "dry run - does not insert in kafka")
        ;
    ...
    if (vm.count("delkeyfile"))
    {
        filename = vm["delkeyfile"].as<std::string>();

        if (!boost::filesystem::exists(filename))
        {
            std::cout << "delkeyfile " << delkeyfile << " does not exists " << std::endl;
            return -1;
        }
    }
    ...
    auto base_name = schema.root()->name();
    ...
    */
    ...
    auto keyschema = get_key_schema(base_name, keys, false, dfr.dataSchema());
    ...
    */
    ...
    std::string key_schema_name = schema.root()->name().fullname() + ".key";
